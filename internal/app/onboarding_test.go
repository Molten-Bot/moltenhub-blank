...
func TestDefaultOnboardingStepsForModeExisting(t *testing.T) {
	...
	if got, want := steps[0].Label, "Agent Token"; got != want {
		t.Fatalf("bind label = %q, want %q", got, want)
	}
	if got, want := steps[0].Detail, "Check existing agent token."; got != want {
...
func TestNormalizeOnboardingMode(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "explicit new mode wins", mode: "new", want: OnboardingModeNew},
		{name: "explicit existing mode wins", mode: "existing", want: OnboardingModeExisting},
		{name: "bind token without agent token infers new", bindToken: "bind-123", want: OnboardingModeNew},
		{name: "agent token defaults to existing", agentToken: "agent-123", want: OnboardingModeExisting},
	}
...
func TestOnboardingModeFromToken(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "bind prefix infers new mode", token: "b_token", want: OnboardingModeNew},
		{name: "bind prefix is case insensitive", token: "B_token", want: OnboardingModeNew},
		{name: "target prefix infers existing mode", token: "t_token", want: OnboardingModeExisting},
		{name: "legacy token defaults to existing mode", token: "legacy-token", want: OnboardingModeExisting},
		{name: "empty token defaults to existing mode", token: "", want: OnboardingModeExisting},
	}
